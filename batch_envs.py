...
class LalEnv(object):
    ...
    def __init__(self, dataset, model, epochs, classifier_batch_size, target_precision):
        ...
        self.number_of_classes = np.size(np.unique(self.dataset.train_labels))
        self.episode_qualities = []
        self.episode_losses = []
        self.episode_training_qualities = []
        self.episode_training_losses = []
        self.rewards_bank = []
        self.batch_bank = []
        self.precision_bank = []
    ...
    def reset(self, number_of_first_samples=10, isBatchAgent=False, target_budget=1.0):
        ...
        for epoch in range(self.epochs):
            ...
            self.episode_losses.append(epoch_loss / len(train_loader))
            self.episode_training_qualities.append(val_acc)
        ...
        self.episode_training_losses.append(
            criterion(predictions, torch.tensor(self.dataset.test_labels_one_hot_encoding).float().to(self.device)).item())
    ...
    def step(self, batch_actions_indices):
        ...
        self.episode_losses.append(epoch_loss / len(train_loader))
        self.episode_training_qualities.append(best_val_acc)
        ...
        self.batch_bank.append(len(batch_actions_indices) / len(self.dataset.train_data))
        self.episode_training_losses.append(
            criterion(predictions, torch.tensor(self.dataset.test_labels_one_hot_encoding).float().to(self.device)).item())
...
class LalEnvFirstAccuracy(LalEnv):
    ...
    def reset(self, number_of_first_samples=10, isBatchAgent=False, target_budget=1.0):
        state, next_action = super().reset(number_of_first_samples=number_of_first_samples, isBatchAgent=isBatchAgent,
